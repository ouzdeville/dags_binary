...
void Sq_mod(poly_t p, poly_t * sq, int t){
	...
	poly_t Z_i;
	Z_i = poly_alloc(deg);
	poly_set_coeff(Z_i, 1, gf_unit());
	sq[0] = Z_i ;
	for (i = 1; i < t; i++){
		Z_i= poly_mul(Z_i, Z_i);
		 //poly_rem( Z_i, p);
		sq[i] = Z_i ;
	}
	poly_free(Z_i);
...
void Tr_pol(poly_t * tr,poly_t p, int e, int m){
	...
	//printf("a= %d\n",a);
	//Sq_mod(p, sq, m);
	poly_set_coeff(tr[e],1, a);
	a = gf_square(a);
	for (i = 0; i < m; i++){
		//printf(" %d--a= %d\t",2<< i,a);
		poly_set_coeff(tr[e], 2<< i, a);
		a = gf_square(a);
...
void Trace_pol(poly_t * tr, int e, int m){
	int i;
	gf_t a;
	a = gf_exp[e];
	poly_set_coeff(tr[e],1, a);
...
int BTA_round(poly_t sigma, gf_t * res, poly_t * tr, int e, int d, int num_tr){
	...
	Trace_pol(tr, e, num_tr);
	gcd1 = poly_gcd(tr[e], sigma);
	gcd2 = poly_quo(sigma, gcd1);
	i =poly_calcule_deg(gcd1);

	j = BTA_round(gcd1, res, tr, e + 1, i,num_tr);
  	j+= BTA_round(gcd2, res+j, tr, e + 1, d-i,num_tr);
...
int BTA(poly_t sigma, gf_t * res){
	...
	for (i = 0; i < EXT_DEGREE; ++i){
		tr[i] = poly_alloc(2<<(EXT_DEGREE-1));
	}
	
	int d=poly_calcule_deg(sigma);

	
	int r = BTA_round(sigma , res,  tr, 0, d, EXT_DEGREE);
